IF OBJECT_ID('[unit_tests].[test movies_get get by owned]') IS NOT NULL
    DROP PROCEDURE [unit_tests].[test movies_get get by owned];
GO

-- test that movies_get gets all the owned movies
CREATE PROCEDURE [unit_tests].[test movies_get get by owned]
AS
BEGIN
    --Assemble
    ...
    EXEC dbo.movies_get @id = NULL, @title = NULL, @owned = @owned;
