IF OBJECT_ID('[unit_tests].[test movies_upd valid update]') IS NOT NULL
    DROP PROCEDURE [unit_tests].[test movies_upd valid update];
GO

-- test that movies_upd updates the movie correctly
CREATE PROCEDURE [unit_tests].[test movies_upd valid update]
AS
BEGIN
    --Assemble
    ...
    --Assert
    DECLARE @duration INT = (SELECT TOP(1) duration FROM dbo.movies WHERE id = @id ORDER BY id);

    EXEC tSQLt.assertEquals @Expected = @duration, @Actual = @correct_duration, @Message = 'Update duration value not correct.';
END;
