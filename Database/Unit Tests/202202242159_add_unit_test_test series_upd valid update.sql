IF OBJECT_ID('[unit_tests].[test series_upd valid update]') IS NOT NULL
    DROP PROCEDURE [unit_tests].[test series_upd valid update];
GO

-- test that series_upd updates the series correctly
CREATE PROCEDURE [unit_tests].[test series_upd valid update]
AS
BEGIN
    --Assemble
    ...
    --Assert
    DECLARE @title [VARCHAR](200) = (SELECT TOP (1) title FROM dbo.series WHERE id = @id ORDER BY id);
