IF OBJECT_ID('[unit_tests].[test series_episode_del valid delete]') IS NOT NULL
    DROP PROCEDURE [unit_tests].[test series_episode_del valid delete];
GO

-- test that serie_episode_del deletes the series episode correctly
CREATE PROCEDURE [unit_tests].[test series_episode_del valid delete]
AS
BEGIN
    --Assemble
    ...
    --Act
    EXEC dbo.series_episode_del @id = @id

    --Assert
    DECLARE @deleted BIT = CASE WHEN EXISTS((SELECT TOP (1) title FROM dbo.series_episode WHERE id = @id ORDER BY id)) THEN 1 ELSE 0 END;
